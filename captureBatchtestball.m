...
files(n).trackpts = 'DLTdv5_testballxypts.csv';%specific path and file name
files(n).soundfile = '';
files(n).headbar = 0;
...
files(n).group=1;
files(n).sex=1; 
files(n).FrameS=310;%frame where cricket is first available
files(n).FrameEnd=540;%frame where cricket is caught
files(n).CapTime=9;%capture time in seconds
files(n).FrameFlash=''; %unused, left over from earlier AV sync method
files(n).Moviefile='testball.mp4';
