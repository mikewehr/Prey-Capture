function FileListBuilder(varargin)
% Prey Capture data file list builder
% modified from CellListBuilder 05-2019
%
%
% what does it do? It creates/appends a machine readable list of files
% this will be a text file with each datafile and perhaps other stuff.
...
global P
%do we want .avi files, .mat files, .csv files, or other?
ext='.avi';
...
if isempty(d)
    h = errordlg(sprintf('no %s files found in this directory.', ext), 'no datafiles');
else
    SelectCells(d)
end
